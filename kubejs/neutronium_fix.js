// Replace kubejs/server_scripts/src/Adjust_material.js using this file
...
ServerEvents.recipes(event => {
    ...
    adjust_all('neutronium', 2, 32678, 'avaritia:neutron_ingot', 'avaritia:neutron') // set original ingot and block to avaritia's
    ...
    function adjust_all(name, mutiple, eut, _ingot, _block) {
        //调整全部
        adjust_plate(name, mutiple, eut, _ingot, _block)
        adjust_extractor(name, mutiple, eut, _ingot)
        adjust_rod(name, mutiple, eut, _ingot)
        adjust_wiremill(name, mutiple, eut, _ingot)
    }

    function adjust_plate(name, mutiple, eut, _ingot, _block) {
        ...
        let ingot = _ingot || "gtceu:" + name + "_ingot"
        let plate = "gtceu:" + name + "_plate"
        let block = _block || "gtceu:" + name + "_block"
    ...
    function adjust_extractor(name, mutiple, eut, _ingot) {
        //提取，只提取锭
        let ingot = _ingot || "gtceu:" + name + "_ingot"
    ...
    function adjust_rod(name, mutiple, eut, _ingot) {
    ...
    function adjust_wiremill(name, mutiple, eut, _ingot) {
